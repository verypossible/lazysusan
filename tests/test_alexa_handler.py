...
from lazysusan.alexa_handler import AlexaHandler
...
@pytest.fixture
def app():
    return AlexaHandler(os.path.join(CWD, "states.yml"))


def test_get_user_id_from_event_exception():
    with pytest.raises(Exception) as err:
        AlexaHandler.get_user_id_from_event({})
...
def test_get_user_id_from_session():
    ...
    assert AlexaHandler.get_user_id_from_event(event) == "brianz"
...
def test_get_user_id_from_system():
    ...
    assert AlexaHandler.get_user_id_from_event(event) == "fooey"
